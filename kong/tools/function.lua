...
function Function.trim (s)
    return (string.gsub(s, "^%s*(.-)%s*$", "%1"))
end

function Function.explode(d,p)
    local t, ll
    t={}
    ll=0
...
function Function.lpcall( ... )
    local ok, rs_or_error = pcall( ... )
    if ok then
        return rs_or_error
    else
        Function.raise_syserror(rs_or_error)
    end
end

function Function.raise_syserror(err)
    ngx.say('<pre />')
    local s = Function.sprint_r(err)
    ngx.say(s)
    ngx.eof()
end
...
-- try to require
function Function.try_require(module_name, default)
...
-- read file
function Function.read_file(file_path)
...
-- split function
function Function.split(str, pat)
    local t = {}
    local fpat = "(.-)" .. pat
    local last_end = 1
...
function Function.split_path(str)
    return Function.split(str, '[\\/]+')
end

-- value in table?
function Function.included_in_table(t, value)
    for i = 1, #t do
...
-- reverse table
function Function.reverse_table(t)
    local size = #t + 1
    local reversed = {}
    for i = 1, #t  do
        reversed[size - i] = t[i]
...
-- @return a table
function Function.get_keywords ()
...
function Function.quote_string(s)
...
local function quote (s)
    if type(s) == 'table' then
        return Function.write(s,'')
    else
        --AAS
        return Function.quote_string(s)-- ('%q'):format(tostring(s))
    end
end
...
local function quote_if_necessary (v)
    if not v then return ''
        ...
        --AAS
        if v:find ' ' then v = Function.quote_string(v) end
    end
    return v
end
...
function Function.write (tbl,space,not_clever)
    ...
    if not keywords then
        keywords = Function.get_keywords()
    end
    local set = ' = '
...
function Function.sprint_r(o)
    return Function.write(o)
end

-- get the lua module name
function Function.get_lua_module_name(file_path)
...
function Function.basename(str)
    local name = sgsub(str, "(.*/)(.*)", "%2")
    return name
end

function Function.dirname(str)
    if str:match(".-/.-") then
